...
- スイッチプレートを SVG/DXF で書き出し（スイッチ穴 + 外周）
  - 外周は「凸包 + マージン」の外形ポリゴン
- ケース外形を SVG/DXF で書き出し（PCB外形オフセット + 任意でSW穴）
...
5. `実行`（SW配置） / `スイッチプレートを書き出し`（SVG/DXF） / `ケース外形を書き出し`（SVG/DXF）

### スイッチプレート外形モード

- `スイッチプレートを書き出し` の外形モードで `四角形 union（ブーリアン）` を選ぶと、凸包よりも外形が追従します（union用マージン(mm)で調整）

### ケース外形（DXF/SVG）

- `ケース外形を書き出し` は、PCB外形（BoardOutline レイヤー11）から
  - `PCB_CLEARANCE` = `+クリアランス(mm)`
  - `CASE_OUTER` = `+クリアランス(mm)+壁厚(mm)`
  を書き出します（DXFはレイヤー名、SVGは data-name で区別）
- 任意で SW 穴も KLE から生成して書き出します
